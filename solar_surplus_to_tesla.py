...
def mainfunction(envoy_data=None):
    ...
    if tesla_api.is_car_home(teslafi_dict):
        if tesla_api.is_car_plugged_in(token, teslafi_dict):
            ...
            if battery_level < int(config['DEFAULT']['minimum_battery_level']): #low battery charge no matter what
                tesla_api.set_charging_amps(token, int(config['DEFAULT']['max_amps']), teslafi_dict)
                tesla_api.start_charge(token, teslafi_dict)
            elif battery_level > int(config['DEFAULT']['max_battery_level']): # high battery stop charging
                ...
                if tesla_api.is_car_charging(teslafi_dict): #car already charging
                    ...
                    if current_amps is None:
                        ...
                        return
                    current_car_consumption = current_amps * int(config['DEFAULT']['voltage'])
